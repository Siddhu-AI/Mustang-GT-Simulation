...
const CarModel: React.FC<CarModelProps> = ({ steering, damage }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (smokeRef.current && damage > 30) {
      ...
      const smokeDensity = (damage - 30) / 70; // 0 to 1
    ...
    <group>
      ...
      <group position={[0, 0, 0]}>
        ...
        <group position={[0, 1.05, 0]}>
          <mesh position={[-0.22, 0, 0.1]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <group position={[0, 0.9, -2.15]}>
          ...
          {/* Mustang Logo Placeholder (Silver Horse) */}
        ...
        <group position={[0, 1.32, -0.4]}>
          <mesh castShadow>
            <boxGeometry args={[1.68, 0.58, 2.0]} />
            {bodyMaterial}
          </mesh>
          <mesh position={[0, 0.05, 1.01]} rotation={[-0.4, 0, 0]}>
        ...
        <group position={[0.78, 0.8, 2.18]}>
           ...
           {[-0.08, 0, 0.08].map((x, i) => (
             <mesh key={i} position={[x, 0, 0.01]} rotation={[0, 0, 0.3]}>
                <boxGeometry args={[0.02, 0.12, 0.01]} />
                <meshStandardMaterial color="#ffffff" emissive="#ffffff" emissiveIntensity={damage > 80 ? 0.5 : 4} />
             </mesh>
           ))}
        </group>
        <group position={[-0.78, 0.8, 2.18]}>
      ...
      {[
        ...
        <group key={idx} position={wheel.pos as any} ref={wheel.ref as any}>
          <mesh rotation={[0, 0, Math.PI / 2]}>
            <cylinderGeometry args={[0.39, 0.39, 0.45, 64]} />
